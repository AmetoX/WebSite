...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['editableParagraph'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM card2 WHERE ID = ?");
    ...
    $stmt->close();
    if($count <= 0){
      $stmt = $conn->prepare("INSERT INTO card2 SET descriere = ?");
      if (!$stmt) {
          die("Error preparing statement: " . $conn->error);
      }
      $stmt->bind_param("s", $content);
    }else{
      $stmt = $conn->prepare("UPDATE card2 SET descriere = ? WHERE id = ?");
      if (!$stmt) {
          die("Error preparing statement: " . $conn->error);
      }
